...
with st.sidebar:
    ...
    if symbol != st.session_state.symbol:
        ...
        if sec_id:
            ...
            # Clear previous data so the condition below fails
            st.session_state.sec_id = None
            st.session_state.expiry = None
    ...
    # Depth display (optional – uses your depth_feed)
    st.divider()
...
if st.session_state.sec_id and st.session_state.expiry:
    ...
    # OI Divergence (sign of OI change vs price change)
    ...
    # OI Shift (change in highest OI strike)
    if prev_df is not None:
    ...
    c4.info(f"📊 ATM PCR: {df.loc[df['Strike']==atm_strike, 'PE OI'].values[0] / df.loc[df['Strike']==atm_strike, 'CE OI'].values[0] if atm_strike in df['Strike'].values else 'N/A'}")
    c5.info(f"🔥 OI Strength: {'High' if pcr > 1.2 or pcr < 0.8 else 'Moderate'}")

    # Level 3 – Option Chain Table (with all advanced columns)
    ...
    # Candlestick chart (using your chart.py)
